...
//            out.println(user.getId());
            Mentee mentee = md.getMenteeById(user.getId());
            HttpSession session = request.getSession();
            session.setAttribute("mentee", mentee);
            response.sendRedirect("home");
        }
        else {
            String error = "New password and password confirm are not the same";
            request.setAttribute("error", error);
            request.getRequestDispatcher("updateNewPass.jsp").forward(request, response);
        }
    }
